...
class RegisterController extends BaseController
{
	...
	public function submit()
	{
		...
		if (empty($username) || empty($pass) || empty($email) || empty($fullname) || empty($phone) || empty($address)) {
			echo "<script>
		...
		if (User::insert($username, $pass, $email, $fullname, '',$phone,$address,2,'active')) {
            ...
            window.location.href = 'index.php?page=main&controller=home&action=index';
          	</script>";
		} else {
			echo "<script>
            alert('Tạo tài khoản thất bại. Vui lòng thử lại sau.');
            window.location.href = 'index.php?page=main&controller=register&action=index';
          	</script>";
		}
		exit();
	}
